'''
Author: BL
Last_date_modified: 2023.06.12
Purpose: To run the Tracer explant dataset that corresponds with the AM-RNA-931 and AM-DNA-343 dataset through Cassiopeia preprocessing and tree reconstruction.

The fastqs for this dataset can be found here: 
    /Genomics/chanlab/blaw/TLS/raw_data/explant/2594__Tracer_TLS_Outgrowth_2__TLS_DBiT-seq_Celeste-read-1.fastq.gz
    /Genomics/chanlab/blaw/TLS/raw_data/explant/2594__Tracer_TLS_Outgrowth_2__TLS_DBiT-seq_Celeste-read-3.fastq.gz
    
These files will be named PZ-2594 to correspond with Pengfei run 2594
...
name = 'PZ-2594_tracer_outgrowth_2'
output_dir = "/Genomics/chanlab/blaw/TLS/sandbox/explant/1_preprocessing/PZ-2594_tracer_outgrowth_2/"
...
# create the file location for this run
cas.pp.setup(output_dir, verbose=verbose)

# Raw fastq's
input_files = ["/Genomics/chanlab/blaw/TLS/raw_data/explant/2594__Tracer_TLS_Outgrowth_2__TLS_DBiT-seq_Celeste-read-1.fastq.gz",
              "/Genomics/chanlab/blaw/TLS/raw_data/explant/2594__Tracer_TLS_Outgrowth_2__TLS_DBiT-seq_Celeste-read-3.fastq.gz"]

# Create the bam file from the fastqs
bam_fp = cas.pp.convert_fastqs_to_unmapped_bam(input_files, chemistry='10xv3', output_directory=output_dir, name=name, n_threads=n_threads)

# Performs initial filtering on the BAM file. Each base of the UMI must pass the PHRED score to pass the filtering.
bam_fp = cas.pp.filter_bam(bam_fp, output_directory = output_dir, quality_threshold = 10, n_threads = n_threads)
