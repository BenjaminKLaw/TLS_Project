...
Returns:
    QC plots of the preprocessing metrics
'''
...
name = 'AM-DNA-098'
output_dir = '/Genomics/chanlab/blaw/TLS/data/AM-DNA-098/lineage/1_preprocessing/QC/'
umi_table_loc = '/Genomics/chanlab/blaw/TLS/data/AM-DNA-098/lineage/1_preprocessing/umi_table_error_correct.txt'
...
plt.title('TLS2 Reads per UMI')
plt.ylabel('UMI Count')
plt.xlabel('Number of Reads')
#plt.yscale('log')
plt.savefig(output_dir + 'reads_per_UMI.pdf', dpi = 300)
plt.close()
...
plt.title('TLS2 UMI per CellBC')
plt.ylabel('Number of UMI')
plt.xlabel('Rank Order')
plt.xscale('log')
plt.yscale('log')
plt.savefig(output_dir + 'UMI_per_cellBC.pdf', dpi = 300)
plt.close()
...
plt.title('TLS2 UMI per intBC')
plt.ylabel('intBC Count')
plt.xlabel('Number of UMIs')
#plt.yscale('log')
plt.savefig(output_dir + 'UMI_per_intBC.pdf', dpi = 300)
plt.close()
...
plt.title('TLS2 UMIs per intBC')
ax.set_xticklabels(ax.get_xticklabels(), rotation = 90)
ax.set_ylabel('intBC Count')
plt.savefig(output_dir + 'UMI_per_Target_Site.pdf', bbox_inches='tight', dpi = 300)
plt.close()
...
plt.title('TLS2 Mean UMI vs % Cells')
plt.ylabel('% of Cells')
plt.xlabel('Mean UMIs per intBC')
#plt.yscale('log')
plt.savefig(output_dir + 'Mean_UMI_vs_Percent_Cells.pdf', dpi = 300)
plt.close()