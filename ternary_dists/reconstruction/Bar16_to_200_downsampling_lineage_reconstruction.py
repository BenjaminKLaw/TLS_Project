...
for i in range(15):
    name = '{}_Bar16_to_200'.format(i)
    temp_allele = pd.read_csv('/Genomics/chanlab/blaw/TLS/sandbox/ternary_studies/downsampling_to_200/Bar16_to_200/allele_tables/{}_downsample_allele_table.txt'.format(name), sep = '\t', index_col = 0)

    temp_allele = remove_NaN(temp_allele)

    hybrid_tree_pipeline(temp_allele, name, total_priors, '/Genomics/chanlab/blaw/TLS/sandbox/ternary_studies/downsampling_to_200/Bar16_to_200/trees/')